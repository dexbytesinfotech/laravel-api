    ...
    public function handle(InstantPushNotification $event)
    {
        if (isset($event->message['body']) && $event->message['body'] != '') {
            ...
            $messageData = [
                ...
                "with_image" => 0,
                "is_silent" => isset($event->message['is_silent']) ? $event->message['is_silent'] : 1,
            ...
            if (isset($event->message['role'])) {
                $pushDevices  = $pushDevice->with('user')->whereIn('user_id', $userIds)
                ->whereNotNull('device_token_id')->where('status','active')->where('app_name',$event->message['role'])
                ->get()->toArray();
            }else {
                $pushDevices  = $pushDevice->with('user')->whereIn('user_id', $userIds)
            ...
            if (!empty($pushDevices)) {
                ...
                $is_silent = isset($event->message['is_silent']) ? $event->message['is_silent'] : 0;
                $data = [
